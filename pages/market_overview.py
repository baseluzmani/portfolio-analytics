"""Market Overview page — all tracked instruments at a glance."""
    ...
    return html.Div([
        html.Div([
            html.Div([
                ...
                html.P(
                    "All tracked instruments. Click rows to select for the chart.",
        ...
        dcc.Store(id='market-selected-funds', data=[]),
        dcc.Store(id='market-sort-state', data={'col': 'YTD', 'asc': False}),
        dcc.Interval(id='market-auto-refresh', interval=60*60*1000, n_intervals=0),
    ])


@callback(
    Output('market-selected-funds', 'data', allow_duplicate=True),
...
@callback(
    Output('market-relative-chart', 'figure'),
...
def update_market_chart(selected_funds, since_date):
    ...
    if not selected_funds:
        fig = go.Figure()
        fig.add_annotation(
            text="Click an instrument in the table to add it to the chart",
        ...
        return fig, "No instruments selected", selected_funds
    
    info = f"{count} instrument{'s' if count != 1 else ''} selected"
